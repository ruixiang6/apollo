...
static void LTDC_Switch(uint8_t sw)
{
	if (sw == 1)
...
//sw:1 打开;0关闭
static void LTDC_Layer_Switch(uint8_t layerx, uint8_t sw)
{
	if (sw == 1)
...
//返回值:0,成功;1,失败。
static uint8_t LTDC_Clk_Set(uint32_t pllsain, uint32_t pllsair, uint32_t pllsaidivr)
{
...
//sx,sy:起始坐标
//width,height:宽度和高度
static void LTDC_Layer_Window_Config(uint8_t layerx, uint16_t sx, uint16_t sy, uint16_t width, uint16_t height)
{
...
//返回值:无
static void LTDC_Layer_Parameter_Config(uint8_t layerx, uint32_t bufaddr, uint8_t pixformat, uint8_t alpha, 
...
static void hal_lcd_led(uint8_t status)
{
	if (status > 0)
...
//设置LCD显示方向
//dir:0,竖屏；1,横屏
static void hal_lcd_display_dir(uint8_t dir)
{
...
//x,y:坐标
//POINT_COLOR:此点的颜色
void hal_lcd_draw_point(uint16_t x, uint16_t y)
{
...
//x,y:坐标
//color:颜色
void hal_lcd_draw_color_point(uint16_t x, uint16_t y, uint32_t color)
{	   
...
//(sx,sy),(ex,ey):填充矩形对角坐标,区域大小为:(ex-sx+1)*(ey-sy+1)   
//color:要填充的颜色
void hal_lcd_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t color)
{          
...
//(sx,sy),(ex,ey):填充矩形对角坐标,区域大小为:(ex-sx+1)*(ey-sy+1)   
//color:要填充的颜色
void hal_lcd_color_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t *color)
{  
...
//x1,y1:起点坐标
//x2,y2:终点坐标  
void hal_lcd_draw_line(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
	uint16_t t; 
	...
	{  
		hal_lcd_draw_point(uRow, uCol);//画点 
		xerr += delta_x ; 
		yerr += delta_y ; 
		if (xerr > distance) 
		{ 
...
//画矩形	  
//(x1,y1),(x2,y2):矩形的对角坐标
void hal_lcd_draw_rect(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
	hal_lcd_draw_line(x1, y1, x2, y1);
	hal_lcd_draw_line(x1, y1, x1, y2);
	hal_lcd_draw_line(x1, y2, x2, y2);
	hal_lcd_draw_line(x2, y1, x2, y2);
}
...
//(x,y):中心点
//r    :半径
void hal_lcd_draw_circle(uint16_t x0, uint16_t y0, uint8_t r)
{
	int32_t a,b;
	int32_t di;
	...
	while (a <= b)
	{
		hal_lcd_draw_point(x0 + a, y0 - b);             //5
 		hal_lcd_draw_point(x0 + b, y0 - a);             //0           
		hal_lcd_draw_point(x0 + b, y0 + a);             //4               
		hal_lcd_draw_point(x0 + a, y0 + b);             //6 
		hal_lcd_draw_point(x0 - a, y0 + b);             //1       
 		hal_lcd_draw_point(x0 - b, y0 + a);             
		hal_lcd_draw_point(x0 - a, y0 - b);             //2             
  		hal_lcd_draw_point(x0 - b, y0 - a);             //7     
...
void hal_lcd_show_char(uint16_t x, uint16_t y, uint8_t num, uint8_t size, uint8_t mode)
{  							  
    uint8_t temp, t1, t;
	uint16_t y0 = y;
	...
	{   
		...
		{			    
			if (temp & 0x80)
				hal_lcd_draw_color_point(x, y, point_color);
			else if (mode == 0)
				hal_lcd_draw_color_point(x, y, back_color);
...
//color:颜色 
//num:数值(0~4294967295);	 
void hal_lcd_show_num(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size)
{         	
	uint8_t t, temp;
	uint8_t enshow = 0;
	...
	for (t = 0; t < len; t++)
	{
		...
		if (enshow == 0 && t < (len - 1))
		{
			if (temp == 0)
			{
				hal_lcd_show_char(x + (size / 2) * t, y, ' ', size, 0);
				continue;
			}
			else
			{
	 	...
	 	hal_lcd_show_char(x + (size / 2) * t, y, temp + '0', size, 0); 
	}
} 
...
//[6:1]:保留
//[0]:0,非叠加显示;1,叠加显示.
void hal_lcd_show_xnum(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint8_t mode)
{  
	uint8_t t,temp;
	uint8_t enshow = 0;
	...
	for (t = 0; t < len; t++)
	{
		...
		if (enshow == 0 && t < (len - 1))
		{
			...
			{
				if (mode & 0X80)
					hal_lcd_show_char(x + (size / 2) * t, y, '0', size, mode & 0X01);  
				else 
					hal_lcd_show_char(x + (size / 2) * t, y, ' ', size, mode & 0X01);  
	 	...
	 	hal_lcd_show_char(x + (size / 2) * t, y, temp + '0', size, mode & 0X01); 
	}
} 
...
//size:字体大小
//*p:字符串起始地址		  
void hal_lcd_show_string(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t size, uint8_t *p)
{         
	uint8_t x0 = x;
    ...
    {       
        ...
        hal_lcd_show_char(x, y, *p, size, 0);
