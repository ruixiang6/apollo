...
u8 exfuns_init(void)
{
	...
	{
		fs[i]=(FATFS*)heap_alloc(sizeof(FATFS),1);	//为磁盘i工作区申请内存	
		if(!fs[i])break;
	}
	file=(FIL*)heap_alloc(sizeof(FIL),1);		//为file申请内存
	ftemp=(FIL*)heap_alloc(sizeof(FIL),1);		//为ftemp申请内存
	fatbuf=(u8*)heap_alloc(512,1);				//为fatbuf申请内存
