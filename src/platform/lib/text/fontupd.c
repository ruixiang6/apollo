...
u32 fupd_prog(u16 x,u16 y,u8 size,u32 fsize,u32 pos)
{
	...
	if(t!=prog)
	{
		hal_lcd_show_string(x+3*size/2,y,240,320,size,"%");		
		t=prog;
		if(t>100)t=100;
		hal_lcd_show_num(x,y,t,3,size);//显示数值
	}
	return 0;					    
} 
//更新某一个
...
u8 updata_fontx(u16 x,u16 y,u8 size,u8 *fxpath,u8 fx)
{
	...
	fftemp=(FIL*)heap_alloc(sizeof(FIL),1);	//分配内存	
	if(fftemp==NULL)rval=1;
	//tempbuf=mymalloc(SRAMIN,4096);				//分配4096个字节空间
	tempbuf=heap_alloc(4096,1);				//分配4096个字节空间
 	...
 	if(rval==0)	 
	{
		...
		while(res==FR_OK)//死循环执行
		{
			...
			hal_spi_flash_write(tempbuf,offx+flashaddr,4096);		//从0开始写入4096个数据  
	  		offx+=bread;	  
...
{	
	...
	//fftemp=(FIL*)mymalloc(SRAMIN,sizeof(FIL));	//分配内存	
	pname=heap_alloc(100,1);	//申请100字节内存  
	buf=heap_alloc(4096,1);	//申请4K字节内存  
	fftemp=(FIL*)heap_alloc(sizeof(FIL),1);	//分配内存	
	...
	if(rval==0)				//字库文件都存在.
	{  
		hal_lcd_show_string(x,y,240,320,size,"Erasing sectors... ");//提示正在擦除扇区	
		for(i=0;i<FONTSECSIZE;i++)			//先擦除字库区域,提高写入速度
		{
			fupd_prog(x+20*size/2,y,size,FONTSECSIZE,i);//进度显示
			hal_spi_flash_read((u8*)buf,((FONTINFOADDR/4096)+i)*4096,4096);//读出整个扇区的内容
			for(j=0;j<1024;j++)//校验数据
			{
				if(buf[j]!=0XFFFFFFFF)break;//需要擦除  	  
			}
			if(j!=1024)hal_spi_flash_erasesector((FONTINFOADDR/4096)+i);	//需要擦除的扇区
		}
		for(i=0;i<5;i++)	//依次更新UNIGBK,GBK12,GBK16,GBK24,GBK32
		{
			hal_lcd_show_string(x,y,240,320,size,UPDATE_REMIND_TBL[i]);		
			strcpy((char*)pname,(char*)src);				//copy src内容到pname
			strcat((char*)pname,(char*)GBK_PATH[i]); 		//追加具体文件路径 
		...
		//全部更新好了
		ftinfo.fontok=0XAA;
		hal_spi_flash_write((u8*)&ftinfo,FONTINFOADDR,sizeof(ftinfo));	//保存字库信息
	}
...
{		
	...
	{
		t++;
		hal_spi_flash_read((u8*)&ftinfo,FONTINFOADDR,sizeof(ftinfo));//读出ftinfo结构体数据
